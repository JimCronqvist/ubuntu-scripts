...
Script options:
  ...
  --if-not-exists          Use CREATE DATABASE IF NOT EXISTS.
...
Examples:
  $SCRIPT_NAME my_app -- --login-path=local
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case "$1" in
            ...
            --charset)
                [[ $# -ge 2 ]] || error "--charset requires a value."
                CHARSET="$2"
                shift 2
                ;;
            ...
            --collation)
                [[ $# -ge 2 ]] || error "--collation requires a value."
                COLLATION="$2"
                shift 2
                ;;
...
main() {
    ...
    local if_not_exists_sql=""
    if [[ "$IF_NOT_EXISTS" -eq 1 ]]; then
        if_not_exists_sql=" IF NOT EXISTS"
    fi

    local sql
    sql="CREATE DATABASE${if_not_exists_sql} ${quoted_db} DEFAULT CHARACTER SET ${CHARSET} DEFAULT COLLATE ${COLLATION};"
    ...
    confirm "Create database '$DB_NAME'?"
