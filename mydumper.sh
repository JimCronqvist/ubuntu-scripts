...
declare -A PARAMS=(
    ...
    ["trx-tables"]=true # If you only use InnoDB. Equivalent to --single-transaction. Rely on InnoDB MVCC.
    ["sync-thread-lock-mode"]="LOCK_ALL" # Consistent backup on RDS including DDLs, if you only need row consistency (you are okay with DDLs messing it up) you can use NO_LOCK. Which generally would be fine.
